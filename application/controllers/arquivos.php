...
class Arquivos extends CI_Controller {
	...
	public function __construct(){
        ...
        $this->data['menuArquivos'] = 'Archivos';
	}
    ...
    public function editar() {
        ...
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'eArquivo')){
          $this->session->set_flashdata('error','No tiene permiso para editar archivos.');
          redirect(base_url());
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->arquivos_model->edit('documentos', $data, 'idDocumentos', $this->input->post('idDocumentos')) == TRUE) {
                $this->session->set_flashdata('success','Modificaciones efectuadas con éxito!');
    ...
    public function excluir(){
        ...
        if($this->db->delete('documentos')){
	    	...
	    	$this->session->set_flashdata('success','Archivo eliminado con éxito!');
	        redirect(base_url() . 'index.php/arquivos/');
        }
        else{

        	$this->session->set_flashdata('error','Ocurrio un error al intentar eliminar el archivo.');
            redirect(base_url() . 'index.php/arquivos/');
        }
