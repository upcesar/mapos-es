        ...
        if ($this->form_validation->run('vendas') == false) {
           $this->data['custom_error'] = (validation_errors() ? true : false);
            ...
            if (is_numeric($id = $this->vendas_model->add('vendas', $data, true)) ) {
                $this->session->set_flashdata('success','Venta iniciada correctamente, agregar productos.');
    ...
    public function faturar() {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            if ($this->vendas_model->add('lancamentos',$data) == TRUE) {
                ...
                $this->session->set_flashdata('success','Venta facturada con éxito!');
                $json = array('result'=>  true);
                echo json_encode($json);
                die();
            } else {
                $this->session->set_flashdata('error','Se produjo un error al intentar facturar la venta.');
